...
export async function detectCaptcha(page: puppeteer.Page): Promise<boolean> {
...
export async function generateCoverLetter(payload: ApplyPayload): Promise<string> {
...
export async function fill(page: puppeteer.Page, sel: string, val: string) {
...
export async function tryFill(page: puppeteer.Page, selectors: string[], val: string) {
...
export async function uploadResume(page: puppeteer.Page, resumePath: string) {
  if (!resumePath) return;
  const fileInput = await page.$('input[type="file"]').catch(() => null);
  if (fileInput) await fileInput.uploadFile(resumePath);
}

export async function fillForm(page: puppeteer.Page, ats: string, payload: ApplyPayload, coverLetter: string) {
...
export async function applyToJob(
  ...
  try {
    ...
    if (captchaAfter) {
      ...
      return { success: false, ats, coverLetter };
    }

    // Step 7: Submit or wait
    if (autoSubmit) {
      send({ type: 'status', step: 5, message: 'Attempting to auto-submit form...' });
      const submitBtn = await page.$(
        'button[type="submit"], input[type="submit"], button[class*="submit" i], button[class*="apply" i]'
      ).catch(() => null);

      if (submitBtn) {
        await submitBtn.click();
        await page.waitForNavigation({ timeout: 10000, waitUntil: 'networkidle2' }).catch(() => {});
        send({ type: 'status', step: 6, message: '✅ Application submitted successfully!' });
        return { success: true, ats, coverLetter };
      } else {
        send({ type: 'status', step: 5, message: '⚠️ Submit button not found. Waiting 30s for manual submission...' });
        await new Promise(r => setTimeout(r, 30000));
        return { success: true, ats, coverLetter };
      }
    } else {
      // Wait for user to review and submit manually
